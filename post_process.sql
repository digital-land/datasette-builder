...
SELECT
    ...
    json_object('type', 'Feature', 'id', g.rowid, 'properties', json_patch( json_object('name', g.name, 'type', g.type, 'slug', s.slug, 'rowid', g.rowid, 'entry-date', entry_date, 'start-date', start_date, 'end-date', end_date), json_group_object(metric.field, metric.value) ), 'geometry', json(AsGeoJSON(GeomFromText(g.geometry, 4326)))) AS geojson_full,
    g.type AS type,
...
SELECT
    ...
    json_object('type', 'Feature', 'id', g.rowid, 'properties', json_patch( json_object('name', g.name, 'type', g.type, 'slug', s.slug, 'rowid', g.rowid, 'entry-date', entry_date, 'start-date', start_date, 'end-date', end_date), json_group_object(metric.field, metric.value) ), 'geometry', json(AsGeoJSON(GeomFromText(g.point, 4326)))) AS geojson_full,
    g.type AS type,
