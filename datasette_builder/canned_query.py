...
def generate_model_canned_queries():
    canned_queries = {}
    schema_models = (c for c in Base.__subclasses__() if hasattr(c, 'dl_type') and c.dl_type == "schema")
    for model in schema_models:
        ...
        for related_class in related_classes:
            if any(True for rel in excluded_relationships if rel in related_class.__tablename__):
                continue
...
def generate_get_id_query(model):
    ...
    return {
        "sql": sql_query,
        "title": f"Get {model.__tablename__} id by reference"
    }
...
def generate_join_query(original_model, related_model):
    ...
    if related_model.dl_type == "join":
        ...
        related_schemas = [rel.mapper.class_ for rel in relationships if rel.mapper.class_ is not original_model]
        if len(related_schemas) != 1:
            raise Exception("Expected join class {} to provide relationship for exactly two tables".format(related_model))
